...
mdns::engine::PingTool::pingIpAddress(const std::string& ipAddress)
{
	...
	if (ipAddress.find(':') != std::string::npos) {
		pingFunction = [this](std::string const& addr, std::stop_token const& token) {
			pingIpv6(addr, token);
		};
	} else {
		pingFunction = [this](std::string const& addr, std::stop_token const& token) {
...
mdns::engine::PingTool::ping(const std::string& command, std::stop_token const& token)
{
	...
	while (!token.stop_requested()) {
		...
		while ((pos = pending.find('\n')) != std::string::npos) {
			...
			m_output += line + "\n";
			if (line.find("Reply from") != std::string::npos || line.find("Request timed out") != std::string::npos)
			{
				m_stats.send++;
			}
...
void 
mdns::engine::PingTool::pingIpv4(const std::string& ipAddress, std::stop_token const& token)
{
#ifdef WIN32
...
void 
mdns::engine::PingTool::pingIpv6(const std::string& ipAddress, std::stop_token const& token)
{
#ifdef WIN32
